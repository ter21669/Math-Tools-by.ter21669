...
int main (void) {
  ...
  double dxx1, dxx2, dxx, dyy1, dyy2, dyy, kaix, kaiy, one, xxx, yyy, ttt;
  char equ1[50], equ2[50], ans;
  
  printf("連立方程式ツール(二元一次) : 連立方程式の解を算出します - VER 1.0.3\n\n");
...
ttt = (t1 * one) / (t2 * one);
if (xxx == yyy && yyy == ttt) {
  printf("Error 4 : その連立方程式は解が一意に定まりません。\n\n");
}
else if (xxx == yyy && yyy != ttt) {
  printf("Error 5 : その連立方程式には解が存在しません。\n\n");
}
else {
